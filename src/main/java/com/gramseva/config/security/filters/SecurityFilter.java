...
public class SecurityFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        if (token != null) {
            token = this.appUtils.getTokenFromHeader(token);
            if (!this.isPublic(request.getRequestURI())) {
                if (!this.isValid(token)) {
                    this.handleException(request, response, Constants.INVALID_TOKEN);
                    return;
                }
            }
    ...
    public void handleException(HttpServletRequest request, HttpServletResponse response, String message) {
